...
CONTAINS
  ...
  SUBROUTINE getFullKpointList(K, R, kLVshift, KpList, eps_)
    ...
    call generateFullKpointList(K, R, kLVshift, temp_KpList, eps_=eps)
  ...
  !!<parameter name="eps" regular="true">Floating point tollerance.</parameter>
  SUBROUTINE getIrredKpoints(A, AtBas, at, K, R ,kLVshift, IrrKpList, weights, eps_)
    ...
    call generateIrredKpointList(A, temp_AtBas, at, K, R, kLVshift, temp_IrrKpList, temp_weights, eps_=eps)
