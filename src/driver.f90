PROGRAM kpoint_driver
  ...
  R = transpose(reshape((/1.0_dp, 0.0_dp, 0.0_dp, &
                 ...
                 0.0_dp,  0.0_dp, 1.0_dp/),(/3,3/))) !tests 1, 2
...
!  write(*,'(3("PP: ",3(1x,f7.3),/))') matmul(K,(/1,0,0/))
  ...
  shift  =  (/0.0_dp,0.0_dp,0.0_dp/) !test 4
...
!!!    shift  =  (/0.00_dp,0.00_dp,0.00_dp/)
  ...
  call pysave(K, "K.in.1")
  call pysave(R, "R.in.1")
  call pysave(shift, "shift.in.1")
  call pysave(klist, "unreduced_klist.in.1")
  call pysave(pgOps, "symops.in.1")
  call symmetryReduceKpointList(K, R, shift,  klist, pgOps, rdKlist, weights, eps)
  call pysave(rdKlist, "simple_cubic_kpts.out.1")
  call pysave(weights, "simple_cubic_wts.out.1")
